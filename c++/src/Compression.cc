...
DIAGNOSTIC_PUSH
  ...
  public:
     ...
     * 1. The seeked position is not read yet from the inputstream.
     * 2. It is already read from the input stream, but has not been
     *    decompressed yet, ie. it's not in the output stream.
     * 3. It is already read and decompressed into the output stream.
     */
  ...
  private:
    ...
    // The remaining size of the current chunk that is not yet uncompressed.
...
DIAGNOSTIC_POP
  ...
  bool ZlibDecompressionStream::Next(const void** data, int*size) {
    // If we are strating a new header, we will have to store its positions
    // after decompressing.
  ...
  void ZlibDecompressionStream::seek(PositionProvider& position) {
    ...
    // Clear state to force the following Next to read from the right position.
    state = DECOMPRESS_HEADER;
    outputBuffer = nullptr;
    outputBufferLength = 0;
    remainingLength = 0;
